...
import type { ClassManagement, UpdateClassRequest, Address, RecurringPattern } from '../../../services';
...
interface FormData extends Omit<UpdateClassRequest, '_id'> {
  _id: string;
}
...
const EditClassModal: React.FC<EditClassModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (isOpen && classData) {
      ...
      setFormData({
        ...
        objectives: classData.objectives.length > 0 ? classData.objectives : [''],
        prerequisites: classData.prerequisites.length > 0 ? classData.prerequisites : [''],
        requiredMaterials: classData.requiredMaterials.length > 0 ? classData.requiredMaterials : [''],
        classPrice: classData.classPrice,
        currency: classData.currency,
        tags: classData.tags.length > 0 ? classData.tags : ['']
      });
    }
  }, [isOpen, classData]);
    ...
    switch (currentStep) {
      ...
      case 3:
        return !!(
          ...
          formData.tags && formData.tags.some(tag => tag.trim() !== '')
        );
      case 4:
        return true;
      default:
  ...
  const handleSubmit = async () => {
    try {
      ...
      const cleanedData: UpdateClassRequest = {
        ...formData,
        objectives: formData.objectives ? formData.objectives.filter(obj => obj.trim() !== '') : [],
        prerequisites: formData.prerequisites ? formData.prerequisites.filter(pre => pre.trim() !== '') : [],
        requiredMaterials: formData.requiredMaterials ? formData.requiredMaterials.filter(mat => mat.trim() !== '') : [],
        tags: formData.tags ? formData.tags.filter(tag => tag.trim() !== '') : []
      };

      await classManagementApiService.updateClass(cleanedData);
      onClassUpdated();
      onClose();
    } catch (error) {
  ...
  // Render step content (similar to AddClassModal but with pre-populated data)
      ...
      case 4: {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatPrice(formData.classPrice || 0, formData.currency)}</p>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {formData.scheduledDate ? new Date(formData.scheduledDate).toLocaleDateString('en-IN') : 'Not set'} | {formData.startTime} - {formData.endTime}
                  </p>
                </div>
                <div>
            ...
            <div className="space-y-4">
              {formData.objectives && formData.objectives.filter(obj => obj.trim() !== '').length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              {formData.prerequisites && formData.prerequisites.filter(pre => pre.trim() !== '').length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              {formData.requiredMaterials && formData.requiredMaterials.filter(mat => mat.trim() !== '').length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium text-gray-900 mb-2 flex items-center">
              ...
              {formData.tags && formData.tags.filter(tag => tag.trim() !== '').length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                  <h4 className="font-medium text-gray-900 mb-2 flex items-center">
